...
- Сотрудник активен ровно в одной компании. Это закреплено частичным уникальным индексом `uq_company_members_single_active_employee`. Владельцы и заместители под правило не попадают.
- Внутри компании человек активен ровно в одном отделе (`uq_department_members_one_active_company`).
- Исключение сотрудника владельцем или заместителем закрывает членство в компании и во всех её отделах, снимает доступы к папкам, очищает выбранную активную компанию, отменяет приглашения от этой компании и снимает сопоставление в Bitrix24.
...
- Владелец и заместитель могут приглашать пользователя в компанию только как `employee`.
...
- `department_leader` не может пригласить руководителя другого отдела и не может забрать сотрудника из другого отдела: на это есть запрос на перевод, который решает заместитель. Попытка такого приглашения возвращает `409 department_transfer_required` с `user_uuid` в деталях.

Подтверждения:

- Если приглашаемый уже работает в другой компании или состоит в другом отделе, создание приглашения возвращает `409 target_already_engaged`. Повтор с `acknowledge_current_membership: true` отправляет приглашение.
- Если принимающий приглашение уже работает в другой компании, `accept` возвращает `409 company_membership_conflict` с названием текущей компании. Повтор с `confirm_transfer: true` переводит человека: прежнее членство закрывается в той же транзакции.
...
| POST | `/api/v1/companies/{uuid}/ownership-transfers` | Да | Предложить передачу владения компанией. Доступ: владелец |
| GET | `/api/v1/ownership-transfers/incoming` | Да | Предложения передачи владения для текущего пользователя |
| POST | `/api/v1/ownership-transfers/{transfer_uuid}/accept` | Да | Принять компанию. Прежний владелец становится заместителем |
...
OpenRouter analyzer работает поэтапно (`pipeline_version=universal-staged-v6`,
...
3. разбор инструкций на атомарные требования — одновременно с инвентаризацией;
...
```json
{
  "schema_version": 3,
  "prompt_version": "universal-v3.1",
  "pipeline_version": "universal-staged-v6",
  "summary": "...",
  "purpose": "...",
  "outcome": "...",
  "conversation_types": [],
