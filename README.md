...
**Итог звонка в карточке CRM.** Переключатель подключения «Писать итог звонка в
карточку CRM» (`PUT /api/v1/integrations/{connection_uuid}/crm-notes` с
`{"mode": "off" | "auto"}` и `If-Match`; владелец или зам; по умолчанию `off`)
пишет после анализа комментарий в сделку, лид, контакт или компанию, к которой
относится активность звонка (`crm.activity.get` → `crm.timeline.comment.add`).
Повторный анализ и публикация ревизии QA обновляют тот же комментарий
(`integration_crm_notes`, один на звонок и подключение). Нужно право `crm`:
health-check отдаёт `crm_notes_writable`, старым подключениям требуется
...
| GET | `/api/v1/auth/capabilities` | Нет | `{password_reset_enabled}` — `false`, пока `NOTIFY_SENDER=mock`: ссылку никто бы не получил, и кнопка «Забыли пароль?» скрыта |
| POST | `/api/v1/auth/password-reset/request` | Нет | `{email}` → всегда `202`, есть адрес или нет; не больше трёх запросов в час на адрес и десяти на IP (`auth_rate_counters`: `reset_account`, `reset_ip`). Письмо со ссылкой `{PUBLIC_APP_URL}/reset-password?token=…` на 30 минут уходит через очередь доставки; новый запрос гасит прежние ссылки |
| POST | `/api/v1/auth/password-reset/confirm` | Нет | `{token, new_password}` → `204`; токен одноразовый (`422 invalid_password_reset_token`), хранится только SHA-256; все сессии пользователя отзываются, в колокольчик приходит `password_changed` |
...
В шапке отчёта нет служебных данных сервера (ID звонка и анализа, провайдер,
модель). Текст анализа v3 в отчёте читается как в приложении: без служебных
ссылок модели, спикеры по именам из разметки спикеров (иначе «Спикер A»), в том
числе перед цитатами; «Основания инструкции» — названия инструкций, статусы и
приоритеты рекомендаций — русскими словами, неизвестный статус — «Не указано».
...
| GET/PUT | `/api/v1/notification-subscriptions` | Да | Личная матрица «событие × канал»: `items: [{kind, channel, enabled, available}]`; `kind` — `weekly_digest`, `critical_call_alert`; `channel` — `in_app`, `email`, `telegram`. Строки нет — в приложении включено, снаружи выключено. Включить можно только `in_app` (`422 notification_channel_unavailable` для почты и Telegram) |

**Доставка (фаза 3, на заглушках).** События сначала отмечаются в `delivery_marks` (`digest:{user}:{company|personal}:{iso_week}`, `alert:{call}`), поэтому повторный запуск ничего не повторяет; колокольчик пишется в той же транзакции. Каналы вне приложения идут через очередь `outbound_messages` (уникальный `dedupe_key`, аренда `FOR UPDATE SKIP LOCKED`, экспоненциальная задержка от минуты, после пяти попыток — `dead`); отправитель выбирается `NOTIFY_SENDER` и пока только `mock` — пишет письмо в лог целиком (адрес, заголовок, текст со ссылками), но токен в ссылке (`?token=`) заменяет на `[скрыто]`, и помечает `sent`: страница сброса пароля открыта, и токен из лога позволил бы сменить пароль чужого аккаунта. Ссылки в сообщениях строятся от `PUBLIC_APP_URL`. Сообщение замороженной компании не отправляется (`dead`). В сообщениях только цифры, имена сотрудников, названия критериев и ссылки — без цитат разговора и имён клиентов.

- **Недельная сводка** (`weekly_digest_ready`): воркер раз в полчаса находит тех, у кого сейчас понедельник 09:00–09:59 по часовому поясу профиля (пусто — `Europe/Moscow`), и собирает сводку за прошлую ISO-неделю по каждой компании и личному кабинету. Владельцу, заму и лидеру — звонки и средний балл с дельтой, три слабейших критерия, кто вырос и упал сильнее всех, критичные пропуски, до трёх ссылок «стоит послушать»; сотруднику и личному кабинету — свой балл с дельтой, «исправлено / повторилось / новых», открытые ошибки. Пустая неделя сводки не даёт. В колокольчике — строка «42 звонка, средний балл 71, +3 к прошлой неделе».
- **Письма аккаунту**: ссылка сброса пароля и письмо зарегистрированному пользователю о новом приглашении («Вас пригласили в компанию …») идут в очередь с адресом аккаунта. Содержимое письма с секретом (`sensitive`) стирается после отправки. Токен сброса хранится только хешем, рядом — адрес, с которого пришёл запрос (`password_reset_tokens.requested_ip`), чтобы сброс, которого владелец не просил, можно было отследить. Пока отправитель `mock`, письма никуда не уходят; приглашение по адресу человека без аккаунта в эту версию не входит.
- **Оповещение о провальном звонке** (`critical_call_alert`): после анализа, если пропущен критичный критерий или итоговый балл ниже порога (`critical_alert_threshold`, по умолчанию 50). Получает лидер отдела звонка, без него — зам, без зама — владелец; сотрудники звонка узнают о нём в колокольчике; личный звонок — его владелец. Внутренний звонок не оповещает, звонок без итогового балла — только по критичному пропуску. Не чаще одного раза на звонок.
...
- *Личный* звонок засчитывается только владельцу и никогда не бывает совместным или внутренним. Какой спикер — владелец, известно, только если он сам себя отметил: в `PUT /api/v1/calls/{uuid}/transcription/speakers` `contact_user_uuid` может быть и самим редактором («Это я» во фронте), не только его контактом или участником компании звонка. Тогда у владельца появляются `speaker_key` и `talk_share`: по этому спикеру считаются речь в аналитике и зоны роста при следующем анализе.
...
**Речевые метрики.** Считаются на сервере из таймингов слов активной ревизии расшифровки (`call_speech_metrics`, `call_speaker_speech_metrics`), без модели и кредитов; воркер раз в две минуты измеряет новые расшифровки и новые ревизии, уже измеренная ревизия не пересчитывается. Роли спикеров числа не меняют — от них зависит только, чей спикер сотрудник (`call_subjects.speaker_key`), это читается при запросе. Определения: реплика — слова одного спикера подряд с разрывом меньше 3 с (AWS); доля речи — длительность реплик спикера к сумме всех; самый длинный монолог — самая длинная реплика; темп — слов в минуту внутри своих реплик; вопросы в час — слова с «?», приведённые к длительности разговора; выдержка перед ответом — медиана паузы «собеседник закончил → спикер начал»; долгие паузы — разрывы между репликами от 4 с; живость — смен говорящего за 5 минут без реплик короче трёх слов. `GET /api/v1/calls/{uuid}` отдаёт блок `speech` (доли — дробью 0–1); `speech` в строках сотрудников — среднее по звонкам, где сотрудник привязан к спикеру, `team.speech` — медиана команды; профиль — `speech.own` и `speech.team_median` (медиана скрыта вместе со сравнением, если в отделе меньше трёх человек). Метрики не входят в балл; перебивания не считаются.
...
Удаление компании начинает этот путь: 30 дней заморозки, затем 30 дней мягкого удаления, затем полная очистка с откреплением сотрудников. Данные можно вынести до этого мастером переноса (`POST /api/v1/company-data-transfers`): звонки и папки с инструкциями переезжают в другую компанию того же владельца одной транзакцией, отдел у них снимается, автор звонка сохраняется. Сотрудники звонка (`call_subjects`) определялись среди людей прежней компании, поэтому перенос их сбрасывает и пишет событие `company_transfer` с прежним составом; воркер фактов аналитики определяет их заново среди сотрудников новой компании, и отмеченный в звонке человек, которого там нет, теряет к нему доступ. Компания в ответах админки приходит вместе с `lifecycle_state`, `freeze_reason` и `restore_used`: панель по ним решает, какие действия на карточке ещё имеют смысл, и гасит остальные вместо того, чтобы предлагать заведомо неуспешную операцию. Тег пустой компании остаётся пустым — прежняя подстановка `@<uuid>` выводила идентификатор там, где ожидается тег.
...
Свободный текст результата (`summary`, `purpose`, `outcome`,
`conversation_types`, `strengths`, `work_on`, тексты рекомендаций, заголовки,
ответы, разборы, пробелы и советы карточек) сохраняется без служебных ссылок
модели: списки ID карточек, требований, рекомендаций и реплик (`u1.7`, `r3`,
`rec2`, `s4.1`) в скобках удаляются, одиночная ссылка заменяется названием
карточки или рекомендации в «ёлочках», ID реплики уходит вместе со словами «в
сегменте» и т. п., имена полей схемы (`assigned_unit`, snake_case) — тоже.
Правила общие с фронтендом (`internal/analysistext`,
`src/shared/lib/analysis-refs.ts`). Ключи (`items[].id`, `recommendations[].id`,
`item_ids`, `criterion_key`, `priority_recommendation_ids`), цитаты `evidence` и
маркеры `{{speaker:KEY}}` не меняются; заголовок критерия оценочной карты
остаётся авторским. Старые записи не переписываются; при чтении их очищают
экспорт отчёта, комментарий в CRM, поиск, Human QA (названия карточек) и зоны
роста, а экран звонка — фронтенд. Developer API
(`GET /api/{sandbox|production}/v2/calls/{call_uuid}/analysis`) отдаёт старые
записи как сохранены.
