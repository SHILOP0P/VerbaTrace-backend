...
func subscriptionColumns(subscriptionAlias string, planAlias string) string {
	return subscriptionAlias + `.subscription_uuid,
	       ...
	       ` + planAlias + `.type,
	       ` + planAlias + `.name,
	       ` + planAlias + `.monthly_price_minor,
	       ` + planAlias + `.currency,
	       ` + planAlias + `.marketing_hours_hint,
	       ` + planAlias + `.monthly_minutes_limit,
	       ` + planAlias + `.monthly_credit_allowance,
...
func scanSubscription(row planScanner) (models.Subscription, error) {
	...
	if err := row.Scan(
		...
		&subscription.Plan.MarketingHoursHint,
		&subscription.Plan.MonthlyMinutesLimit,
		&subscription.Plan.MonthlyCreditAllowance,
		...
		&subscription.Plan.WebhooksEnabled,
