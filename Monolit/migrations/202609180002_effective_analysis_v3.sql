-- +goose Up
-- The effective projection laid published human scores only over criteria_results
-- of schema v2. Schema v3 keeps its cards in items[], so a human review of a v3
-- analysis changed nothing a reader saw except the top-level score. Human QA keys
-- a v3 card by its code when it has one and by its id otherwise.
...
BEGIN
    ...
    IF jsonb_typeof(p_result->'items') = 'array' AND p_result->'criteria_results' IS NULL THEN
        SELECT COALESCE(jsonb_agg(
            CASE
                WHEN c.decision = 'not_applicable' THEN
                    item || jsonb_build_object('status', 'not_applicable', 'score', NULL, 'effective_source', v_source)
                WHEN c.human_score IS NOT NULL AND c.score_max > COALESCE(c.score_min, 0) THEN
                    item || jsonb_build_object(
                        'score', round(((c.human_score - COALESCE(c.score_min, 0)) / (c.score_max - COALESCE(c.score_min, 0))) * 100),
          ...
          INTO v_items
          FROM jsonb_array_elements(p_result->'items') WITH ORDINALITY AS source(item, ordinal)
          LEFT JOIN call_quality_review_criteria c
            ON c.revision_uuid = v_revision_uuid
           AND c.criterion_key = COALESCE(NULLIF(source.item->>'code', ''), source.item->>'id');

        v_result := jsonb_set(p_result, '{items}', v_items, true)
            || jsonb_build_object('effective_source', v_source, 'score_scale', 100);
        IF v_total IS NOT NULL THEN
            v_result := v_result || jsonb_build_object(
                'score', round(v_total),
                'overall_score', round(v_total),
                'overall_score_label', round(v_total)::TEXT || ' / 100'
            );
        END IF;
        RETURN v_result;
    END IF;
